...
`dict_metals`` 这个字典里面是本人计算的一些常见金属的bulk晶格常数，能量，仅供大家消遣，测试。

如果你要通过这个脚本切用于发表文章的slab，运行前，先自己优化一边bulk，然后修改成自己的结果再运行。
...
4）每个slab有4层，如果想切厚点，修改脚本中的 `size = (i, i, 4)`为你想要的数字。如果想切（5x5)，4层的slab，将前面一行的`for i in range(1,4)`改成`for i in range(1,6)`；
